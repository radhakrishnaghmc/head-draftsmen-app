...
describe('collectTenderDocuments', () => {
  it('recursively finds L1/Intimation-named PDFs and HTML inside a picked folder, skipping unrelated KYC documents', () => {
    const found = collectTenderDocuments([root])
  ...
  it('never includes a file whose extension is not .pdf/.html/.htm, even if the name matches', () => {
    const found = collectTenderDocuments([root])
    expect(found.some((f) => f.name === 'L1 notes.txt')).toBe(false)
  })

  it('never reads the unrelated KYC documents in a "Common Documents" subfolder', () => {
    const found = collectTenderDocuments([root])
  ...
  it('returns the real file bytes, not just names', () => {
    const found = collectTenderDocuments([root])
    const l1 = found.find((f) => f.name === 'L1.pdf')!
    expect(Buffer.from(l1.bytes).toString('utf8')).toBe('l1 sheet content 2')
  })

  it('includes a file explicitly picked by path, regardless of its name — the filename filter only applies inside a recursed folder', () => {
    const oddPath = path.join(root, 'unrelated-name-but-explicitly-picked.pdf')
    fs.writeFileSync(oddPath, 'picked directly')
    const found = collectTenderDocuments([oddPath])
    expect(found).toHaveLength(1)
    expect(found[0].name).toBe('unrelated-name-but-explicitly-picked.pdf')
  })

  it('accepts a mix of an explicit file and a folder in one call', () => {
    const oddPath = path.join(root, 'unrelated-name-but-explicitly-picked.pdf')
    fs.writeFileSync(oddPath, 'picked directly')
    const found = collectTenderDocuments([oddPath, path.join(root, '2).709803-LED Lights-Recall')])
    const names = found.map((f) => f.name).sort()
    expect(names).toEqual([
      'L1.pdf',
      'tender.telangana.gov.in_viewIntimationNotice.html',
