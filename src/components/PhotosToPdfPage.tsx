...
import { IconChevronLeft, IconImage, IconFolder, IconDoc, IconEye, IconDownload, IconTrash, IconWarn, IconOpen } from './Icons'

// One picked image, held as a data URL (same-origin, so the canvas we analyse it
// on is never tainted) along with its natural pixel size.
interface Pic {
  id: string
  name: string
  dataUrl: string
...
// from the four corners, then mark every pixel that differs from it by more than a
// threshold as "content". A row/column counts as content only once enough of its
// pixels are content (so specks and JPEG noise don't blow the box out to the full
// frame). The box is padded slightly and mapped back to the full-resolution image.
// Returns the crop rectangle in the ORIGINAL image's pixels, or null when nothing
// clear was found (then the whole image is used unchanged).
function detectContentBox(img: HTMLImageElement): { x: number; y: number; w: number; h: number } | null {
  ...
  // Background colour: median-ish of the four corners (average is fine here).
  ...
  const THRESH = 42 // per-channel-sum difference that counts as "not background"
  ...
  if (box.w >= img.width * 0.97 && box.h >= img.height * 0.97) return null
  return box
}
 ...
 * pages come out tight like a scan. All image work is done here in the renderer
 * (canvas + pdf-lib); the main process only writes the finished bytes to disk.
 */
export default function PhotosToPdfPage({ onBack }: Props) {
  ...
  async function buildPdfBytes(): Promise<Uint8Array> {
    ...
    for (const pic of pics) {
      const { bytes, w: iw, h: ih } = await toJpegBytes(pic.dataUrl, autoCrop)
  ...
  return (
    <>
      ...
      {previewUrl && (
        <div className="editor-overlay" onClick={closePreview}>
