 ...
 */
async function sofficeConvert(soffice: string, input: Buffer, inExt: string, outExt: string): Promise<Buffer> {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'docugen-convert-'))
  try {
    const inputPath = path.join(dir, `source.${inExt}`)
    fs.writeFileSync(inputPath, input)
    await execFileAsync(soffice, ['--headless', '--convert-to', outExt, '--outdir', dir, inputPath], {
      timeout: 60000
    })
 ...
 */
async function convertWithRetry(soffice: string, input: Buffer, inExt: string, outExt: string): Promise<Buffer> {
  let lastErr: unknown
  for (let attempt = 0; attempt < 4; attempt++) {
    try {
      return await sofficeConvert(soffice, input, inExt, outExt)
    } catch (err) {
      lastErr = err
...
export async function docxToPageImages(docxBuffer: Buffer): Promise<Buffer[]> {
  ...
  for (let i = 0; i < pageCount; i++) {
    ...
    const singlePdf = Buffer.from(await single.save())
    images.push(await convertWithRetry(soffice, singlePdf, 'pdf', 'png'))
  }
  return images
}
