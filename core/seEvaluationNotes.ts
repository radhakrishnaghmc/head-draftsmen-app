...
const PL = (html: string): string => `<p style="margin:0 0 6px;line-height:1.45">${html}</p>`
...
// reason.
const TABLE_STYLE = 'width:100%;border-collapse:collapse;margin:8px 0'
const TH = (text: string): string =>
  `<td style="border:1px solid #000;padding:4px 6px;font-weight:bold;text-align:center">${esc(text)}</td>`
...
/** "1)" list-item wrapper matching the office's own Ref-block numbering. */
const refItem = (n: string, html: string): string => `<p style="margin:0 0 2px">${esc(n)} ${html}</p>`

function refBlockHtml(d: SeEvaluationRefs): string {
  return (
    refItem('1)', `Administrative sanction approval of the ${esc(d.asRefLine)}.`) +
...
export function buildBidEvaluationHtml(d: BidEvaluationData): string {
  ...
  return (
    ...
    `<table style="${TABLE_STYLE}"><tr>${TH('Sl No')}${TH('Name of the Bidder')}${TH('Remarks')}</tr>${rows}</table>` +
    P(
...
export function buildAgencyApprovalHtml(d: AgencyApprovalData): string {
  ...
  return (
    ...
    `<table style="${TABLE_STYLE}"><tr>${TH('Sl.No.')}${TH('Contractor/Agency Name')}${TH('ECV(INR)')}${TH(
      'Excess /Less'
    )}${TH('Percentage (%)')}${TH('Amount (INR)')}${TH('Position')}</tr>${rows}</table>` +
    (l1 && l1Pct
      ? P(
